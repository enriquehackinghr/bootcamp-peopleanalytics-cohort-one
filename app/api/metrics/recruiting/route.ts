...
import { getRecruitingPage } from '@/lib/db/metrics'
...
export async function POST(request: Request) {
  try {
    ...
    const data = await getRecruitingPage(parseFilterContext(body.filters))
    return NextResponse.json(data)
  } catch (error) {
    if (error instanceof Error && 'status' in error) return authErrorResponse(error)
    const body: ApiErrorBody = {
      error: error instanceof Error ? error.message : 'Recruiting metrics failed',
    }
